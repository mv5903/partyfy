...
test('User can log in with valid credentials - User has Spotify set up already + no friends', async ({ page }) => {
  await login(page, process.env.EMAIL_2, process.env.PASSWORD_1);
  await expect(page.getByTestId('no-friends-label')).toBeVisible();
});
